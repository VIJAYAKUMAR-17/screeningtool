...
| M18 | Tier 2 discovery quality                     | Fabricated sister entities removed; SEC discovery now parses Exhibit 21 subsidiaries when available; adverse media now requires entity/keyword proximity in the same feed item. Remaining risk: registry depth still depends on public source coverage and OpenCorporates token availability. |
